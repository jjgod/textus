...
- (id)init {
  ...
  if (self) {
    ...
    for (NSString* keyPath in keyPaths)
      [defaults addObserver:self forKeyPath:keyPath options:0 context:nil];
  }
  return self;
}
...
- (void)saveMetaData {
  ...
  [fileURL setUnsignedInteger:lastReadLocation
                  forXattrKey:kLastReadLocationKey];
}
...
#define outputParagraph(output, paragraph)      \
  do {                                          \
    [output appendFormat:@"%@\n\n", paragraph]; \
    paragraph = nil;                            \
  } while (0)
...
- (void)outputTo:(NSMutableString*)output from:(NSString*)contents {
  ...
  for (range = NSMakeRange(0, 0); end < length; range.location = end) {
    [contents getLineStart:&start end:&end contentsEnd:NULL forRange:range];
    lineRange = NSMakeRange(start, end - start);
    line = [[contents substringWithRange:lineRange]
        stringByTrimmingCharactersInSet:newlineCharset];
    if (inParagraph) {
      if ([line length] == 0) {
        inParagraph = NO;
        ...
        outputParagraph(output, paragraph);
        paragraph =
            [[NSMutableString alloc] initWithString:[self firstLine:line]];
      } else
...
- (BOOL)readFromURL:(NSURL*)absoluteURL
  ...
  [self outputTo:wrappedText
            from:[contents stringByReplacingOccurrencesOfString:@"\r"
                                                     withString:@""]];
  [self setFileContentsInPlainText:wrappedText];
  // Remove DOS line endings
  fileContents = [[NSMutableAttributedString alloc]
      initWithString:self.fileContentsInPlainText
          attributes:[self attributesForText]];

  // If "latinFontName" is set, use it as the primary font and leave the regular
  // font as a fallback.
  NSUserDefaults* defaults = [NSUserDefaults standardUserDefaults];
  ...
  if (latinFontName) {
    ...
    NSLinguisticTagger* tagger =
        [[NSLinguisticTagger alloc] initWithTagSchemes:@[ scheme ]
                                               options:options];
    tagger.string = self.fileContentsInPlainText;
    [tagger enumerateTagsInRange:NSMakeRange(
                                     0, [self.fileContentsInPlainText length])
                          scheme:scheme
                         options:options
  ...
  if ([keys containsObject:kLastReadLocationKey])
    lastReadLocation =
        [absoluteURL unsignedIntegerFromXattrKey:kLastReadLocationKey];
...
- (void)observeValueForKeyPath:(NSString*)keyPath
  ...
  if ([keyPath isEqual:@"lineHeight"] || [keyPath isEqual:@"fontName"] ||
